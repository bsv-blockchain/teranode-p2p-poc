...
func NewP2PNode(ctx context.Context, logger *logrus.Logger, config P2PConfig) (*P2PNode, error) {
	...
	if config.PrivateKey == "" {
		...
		if err != nil {
			return nil, fmt.Errorf("[P2PNode] error generating private key", err)
		}
	} else {
		// If a private key is provided, decode it
		pk, err = decodeHexEd25519PrivateKey(config.PrivateKey)
		if err != nil {
			return nil, fmt.Errorf("[P2PNode] error decoding private key", err)
		}
	}
	...
	if config.UsePrivateDHT {
		...
		if err != nil {
			return nil, fmt.Errorf("[P2PNode] error setting up private network", err)
		}
	} else {
		...
		if err != nil {
			return nil, fmt.Errorf("[P2PNode] error creating libp2p host", err)
		}
	}
...
func setUpPrivateNetwork(config P2PConfig, pk *crypto.PrivKey) (host.Host, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[P2PNode] error decoding shared key", err)
	}
	...
	h, err = libp2p.New(opts...)
	if err != nil {
		return nil, fmt.Errorf("[P2PNode] error creating libp2p node", err)
	}
...
func (s *P2PNode) initDHT(ctx context.Context, h host.Host) (*dht.IpfsDHT, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("[P2PNode] error bootstrapping DHT", err)
	}
